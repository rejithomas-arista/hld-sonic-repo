...
- Control plane encap: kernel nexthop object with `encap ip dst <tunnel-dst> src <tunnel-src>`
- No kernel interface created — uses kernel nexthop objects (unlimited scale)
- Routes are configured via FRR static routes referencing the kernel nexthop ID
- **Requires FRR patch** to support `LWTUNNEL_ENCAP_IP` and `ip route ... nhid <N>` CLI
- Use cases: SDN controller-driven traffic steering where the DUT encapsulates and sends to a remote service chain endpoint (firewall, DPI, load balancer) that strips the outer header; large-scale programmatic tunnel provisioning where return traffic takes a different (non-tunnel) path
...
Mode 3 uses kernel nexthop objects instead of kernel interfaces for control plane tunnel representation. Routes referencing the tunnel are configured via FRR static routes using the kernel nexthop ID.

**Nexthop ID allocation:** FRR and the kernel share a single nexthop ID namespace. FRR allocates IDs from 1 upward (wrapping at ~7.3M), and checks for collisions before each allocation. When FRR reads back a kernel-created nexthop, it advances its internal counter past that ID to avoid future conflicts. TunnelIpipMgr lets the **kernel auto-assign** the nexthop ID (by omitting the `id` parameter in `ip nexthop add`) and reads the assigned ID back from the netlink response. This avoids any possibility of ID collision with FRR-allocated nexthops.

```
TunnelIpipMgr         Kernel                FRR (patched)       fpmsyncd             RouteOrch
─────────────         ──────                ─────────────       ────────             ─────────
1. Create SAI
   tunnel + encap
   nexthop

2. Create kernel      ip nexthop add
   nexthop object     id <N> encap ip
                      dst 10.2.0.1          3. Reads NH <N>
                      src 10.1.0.1             back via netlink
                      dev Ethernet0            Parses IPIP encap
                                               Advances id_counter
                                               past <N>
                                                                 onNextHopMsg()
                                                                 stores NH <N>
                                            ...
                                            5. Installs kernel   6. Receives route
                                            route with nhid <N>     with nhid=<N>
...
- **FRR patch (§5.6)** — extended scope:
  1. Parse `LWTUNNEL_ENCAP_IP` in `netlink_nexthop_process_nh()` (already planned)
  2. Add CLI support: `ip route <prefix> nhid <N>` to allow static routes referencing kernel nexthop IDs
...
**Control plane difference from Mode 1:** No kernel tunnel interface. FRR cannot run routing protocols (BGP/OSPF) over the tunnel. FRR can only install static routes via `nhid`. No BFD-driven interface state. Failure detection relies on underlay routing convergence or external controller removing routes.

**Mode 4 flow (symmetric + nexthop):**

Mode 4 combines Mode 3 encap (kernel nexthop object) with Mode 2 decap (SAI tunnel term entry). It provides full bidirectional data plane tunneling at unlimited scale, with control plane encap via nexthop objects.

```
TunnelIpipMgr                    Kernel                  SAI / VPP
─────────────                    ──────                  ─────────
1. Create SAI tunnel             
   + encap nexthop               
   + decap term entry            
...
3. No kernel interface
4. No BFD

Encap: FRR static route nhid <N> → fpmsyncd → RouteOrch → SAI tunnel encap NH (same as Mode 3)
...
1. **Add `parse_encap_ip()` function** (`rt_netlink.c`): Parse `LWTUNNEL_IP_SRC` and `LWTUNNEL_IP_DST` attributes from the `NHA_ENCAP` netlink attribute, storing tunnel source/destination addresses.

2. **Extend `netlink_nexthop_process_nh()`** (`rt_netlink.c`): Add an `else if (encap_type == LWTUNNEL_ENCAP_IP)` branch that calls `parse_encap_ip()` and sets `nh.encap.encap_type = ENCAP_TYPE_IPIP`.

3. **Extend nexthop encap structure** (`zebra_nhg.h`): Add `ENCAP_TYPE_IPIP` to the `encap_type_e` enum and add `tunnel_src`/`tunnel_dst` fields (`struct in6_addr`) to store tunnel endpoints.

4. **Update display code** (`zebra_vty.c`): When `encap_type == ENCAP_TYPE_IPIP`, display tunnel encapsulation info in `show ip route` output.
...
Routing entry for 1.2.3.0/24
  Known via "kernel", distance 0, metric 0, best
  * via <gateway> tunnel encap src 10.1.0.1 dst 10.2.0.1 dev Ethernet0, weight 1
```
